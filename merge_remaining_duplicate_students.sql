...
BEGIN TRY
    ...
    -- Find duplicates by PersonalEmail. 
    -- These are students who registered twice but with slight variations in their names.
    -- We prioritize keeping the one that has an @wigweuniversity.edu.ng email.
    WITH RankedStudents AS (
        SELECT 
            ...
            ROW_NUMBER() OVER(
                ...
                ORDER BY 
                    CASE WHEN OfficialEmail LIKE '%@wigweuniversity.edu.ng%' THEN 0 ELSE 1 END ASC,
                    Id ASC
            ) as RowNum
        FROM Students
        WHERE PersonalEmail IS NOT NULL AND TRIM(PersonalEmail) != ''
    )
    INSERT INTO #StudentMerges (OfficialStudentId, UnofficialStudentId, FirstName, LastName, PersonalEmail)
    SELECT 
        rsPrimary.Id AS OfficialStudentId,
        rsDuplicate.Id AS UnofficialStudentId,
        rsPrimary.FirstName,
        rsPrimary.LastName,
        rsPrimary.PersonalEmail
    FROM RankedStudents rsPrimary
    JOIN RankedStudents rsDuplicate 
        ON LOWER(TRIM(rsPrimary.PersonalEmail)) = LOWER(TRIM(rsDuplicate.PersonalEmail))
        AND rsDuplicate.RowNum > 1
    WHERE rsPrimary.RowNum = 1;

    DECLARE @MergeCount INT = (SELECT COUNT(*) FROM #StudentMerges);
    PRINT 'Found ' + CAST(@MergeCount AS VARCHAR) + ' remaining duplicate student records (by PersonalEmail) to merge.';
    ...
    COMMIT TRANSACTION;
    PRINT 'Merge of ALL remaining duplicate students (by PersonalEmail) completed successfully.';
